...
def gerar_experimentos():
    ...
    r = [list(x) for x in itertools.product(i, j)]
...
class GeradorExperimentos:
    ...
    def initR(self):
        possibles = [i*j for i in range(1, 11)
                     for j in range(10, 1001, 10) if i*j <= 5000]
        sett = set(possibles)
            ...
            for i in self.possibles_r:
                ...
                for j in self.k:
                    ...
                    for l in range(numero_de_iteracoes):
                        ordenador = Experimentos()
                        ordenador.ordenar(mode, self.const_m,
                                          j, i, n, None, True, False)
    ...
    def getBetaDict(self, n: int, mode: str, numero_de_iteracoes: int = 10):
        ...
        with tqdm(total=len(self.m)) as pbar:
            for j in self.m:
                ...
                for l in range(numero_de_iteracoes):
                    ordenador = Ordenador()
                    ordenador.ordenar(mode, j, self.const_k, self.const_r, n, list(
                        np.random.permutation(n)), False, False)
